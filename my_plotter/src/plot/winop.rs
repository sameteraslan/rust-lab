...
impl Winop {
    ...
    pub fn display_with_window(plot: &mut plot::Plot) {
    ...
    pub fn display<T: DisplayablePlot>(plot: &mut T) {
        let base_plot = plot.as_mut_base_plot();
        let mut window = Window::new(
            &base_plot.title,
            base_plot.width as usize,
            base_plot.height as usize,
            WindowOptions {
        ...
        while window.is_open() && !window.is_key_pressed(Key::Escape, minifb::KeyRepeat::No) {
            ...
            if current_size != (base_plot.width as usize, base_plot.height as usize) {
                base_plot.width = current_size.0 as u32;
                base_plot.height = current_size.1 as u32;
            }

            let mut img = Drawing::render(base_plot);
            ...
            if hover_enabled {
                if let Some(mouse_pos) = window.get_mouse_pos(MouseMode::Pass) {
                    ...
                    if let Some((closest_x, closest_y)) =
                        Winop::find_closest_point(base_plot, mouse_x, mouse_y)
                    {
                        ...
                        let scale_x = (base_plot.x_max - base_plot.x_min)
                            / (base_plot.width - 2 * base_plot.margin) as f64;
                        let scale_y = (base_plot.y_max - base_plot.y_min)
                            / (base_plot.height - 2 * base_plot.margin) as f64;

                        let px = ((closest_x - base_plot.x_min) / scale_x + base_plot.margin as f64)
                            as f32;
                        let py = (base_plot.height as f64
                            - base_plot.margin as f64
                            - (closest_y - base_plot.y_min) / scale_y)
                            as f32;
            ...
            window
                .update_with_buffer(&buffer, base_plot.width as usize, base_plot.height as usize)
                .unwrap();
            ...
            if window.is_key_pressed(Key::NumPadPlus, minifb::KeyRepeat::No) {
                Winop::zoom(base_plot, 0.9); // Zoom in
            }
            if window.is_key_pressed(Key::NumPadMinus, minifb::KeyRepeat::No) {
                Winop::zoom(base_plot, 1.1); // Zoom out
            }
        }
    }

    // Finds the closest data point to the given pixel position
    fn find_closest_point(plot: &mut plot::Plot, mouse_x: u32, mouse_y: u32) -> Option<(f64, f64)> {
    ...
    fn zoom(plot: &mut plot::Plot, factor: f64) {
    ...
    pub fn display_real_time<F>(plot: &mut plot::Plot, mut data_generator: F, fps: u64)
    where
        F: FnMut() -> (f64, f64),
    {
        let mut window = Window::new(
        ...
        while window.is_open() && !window.is_key_pressed(Key::Escape, minifb::KeyRepeat::No) {
            ...
            let mut img = Drawing::render(plot); // Render the plot with current settings
